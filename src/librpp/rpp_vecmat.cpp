...
void vec3_array_sum(vec3_t &v_sum2, const vec3_array &va) {
    vec3_clear(v_sum2);
    for (vec3_array_const_iter iter = va.begin(); iter != va.end(); iter++) {
        v_sum2.v[0] += (*iter).v[0];
        v_sum2.v[1] += (*iter).v[1];
        v_sum2.v[2] += (*iter).v[2];
    }
...
void vec3_array_pow2(vec3_array &va) {
    for (vec3_array_iter iter = va.begin(); iter != va.end(); iter++) {
        (*iter).v[0] *= (*iter).v[0];
        (*iter).v[1] *= (*iter).v[1];
        (*iter).v[2] *= (*iter).v[2];
    }
...
void mat33_array_sum(mat33_t &s, const mat33_array &ma) {
    mat33_clear(s);
    for (mat33_array_const_iter iter = ma.begin(); iter != ma.end(); iter++) {
        for (unsigned int c = 0; c < 3; c++) {
            s.m[0][c] += (*iter).m[0][c];
            s.m[1][c] += (*iter).m[1][c];
            s.m[2][c] += (*iter).m[2][c];
...
real_t mat33_det(const mat33_t &a) {
    real_t determinant = a.m[0][0] * a.m[1][1] * a.m[2][2] + a.m[0][1] * a.m[1][2] * a.m[2][0] + a.m[0][2] * a.m[1][0]
            * a.m[2][1] - a.m[2][0] * a.m[1][1] * a.m[0][2] - a.m[2][1] * a.m[1][2] * a.m[0][0] - a.m[2][2] * a.m[1][0]
            * a.m[0][1];
    return (determinant);
}
...
void normRv(vec3_array &normR_v, const vec3_array &v) {
    ...
    for (scalar_array::iterator iter = _l.begin(); iter != _l.end(); iter++) {
...
int solve_polynomial(scalar_array &r_sol, const scalar_array &coefficients) {
    ...
    double dd[5] = { (double) coefficients[0], (double) coefficients[1], (double) coefficients[2],
            (double) coefficients[3], (double) coefficients[4] };
