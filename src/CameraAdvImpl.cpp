...
bool CameraAdvImpl::loadFromFile(const char* filename) {
	...
	char _s[32];
	sprintf(_s, "%c%i%c%c%c", '%', (int) strlen(CAMERA_ADV_HEADER), 's', '\\',
			'n');
	char hdr[MAX_PATH];
	n = fscanf(fp, _s, hdr);
...
void CameraAdvImpl::observ2Ideal(ARFloat ox, ARFloat oy, ARFloat *ix,
		ARFloat *iy) {
	if (undist_iterations <= 0) {
		...
		*iy = oy;
		...
		for (int kk = 0; kk < undist_iterations; kk++) {
			...
			const ARFloat k_radial = 1 + k1 * r_2 + k2 * (r_2_sq) + k3 * (r_2
					* r_2_sq);
			const ARFloat delta_x[2] = { 2 * p1 * x0_x1 + p2
					* (r_2 + 2 * x0_sq), p1 * (r_2 + 2 * x1_sq) + 2 * p2
					* x0_x1 };
			x[0] = xd[0] - delta_x[0];
			x[1] = xd[1] - delta_x[1];
			x[0] /= k_radial;
...
void CameraAdvImpl::ideal2Observ(ARFloat ix, ARFloat iy, ARFloat *ox,
		ARFloat *oy) {
...
bool CameraAdvImpl::changeFrameSize(const int frameWidth, const int frameHeight) {
...
void CameraAdvImpl::printSettings() {
